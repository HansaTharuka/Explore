  ...
  implements SafeParcelable, AppContentCard
{
  ...
  public AppContentCardEntity(AppContentCard paramAppContentCard)
  {
    ...
    for (int k = 0; k < j; k++)
      this.mActions.add((AppContentActionEntity)(AppContentActionEntity)((AppContentAction)localList1.get(k)).freeze());
    List localList2 = paramAppContentCard.kN();
    int m = localList2.size();
    this.XO = new ArrayList(m);
    for (int n = 0; n < m; n++)
      this.XO.add((AppContentAnnotationEntity)(AppContentAnnotationEntity)((AppContentAnnotation)localList2.get(n)).freeze());
    List localList3 = paramAppContentCard.kH();
    int i1 = localList3.size();
    this.XJ = new ArrayList(i1);
    while (i < i1)
    {
      this.XJ.add((AppContentConditionEntity)(AppContentConditionEntity)((AppContentCondition)localList3.get(i)).freeze());
      i++;
    }
  }
